...
class ConceptExtractor(
    ...
    suspend fun extractAndStore(
        ...
        val prompt = PromptTemplates.conceptExtractionPrompt(text)
        ...
        return result.fold(
            ...
            ifRight = { response ->
                parseAndMergeConcepts(response, materialId, chunkIds)
            }
        )
    }
    ...
    private suspend fun parseAndMergeConcepts(
        ...
        for (concept in extracted) {
            ...
            if (existing != null) {
                val updatedMaterialIds = mergeJsonArrays(existing.materialIdsJson, matIdJson)
                val updatedChunkIds = mergeJsonArrays(existing.chunkIdsJson, chunkIdJson)
                val updated = existing.copy(
                    materialIdsJson = updatedMaterialIds,
